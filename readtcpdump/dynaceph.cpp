...
//    struct h_op *tmp_op;
//    u_int32 type;
    unsigned int i,count=1,opslen=0;
    unsigned int check_area=0;
    
    if (NULL==msg)     		
    	return;
    	
    //check available of first part
    check_area=sizeof(struct ceph_msg_header);
	...
	//print object id
	  ...
	  for(i=1;i<opslen+1;i++)
	  {
			switch (p_op->op)
				{
					case C_OSD_OP_READ:		
                        printf("%s,%d,0x%x,%d,%d",prefix,ceph_msg->header.type,ceph_request->osd_flag,ceph_msg->header.tid,ceph_msg->header.front_len);
                        printf(",%d",opslen);
						printf(",%.*s",40,&ceph_request->obj_id[4]);						
...
//	}
	...
	//print object id
	  ...
	  for(i=1;i<reply_head->op_number+1;i++)
	  {
				...
				{
					case C_OSD_OP_READ:		
                        printf("%s,%d,0x%x,%d,%d",prefix,ceph_msg->header.type,reply_head->osd_flag,ceph_msg->header.tid,ceph_msg->header.front_len);	
                        printf(",%d",opslen);	
                        printf(",%.*s",40,&reply_head->obj_id[4]);						
